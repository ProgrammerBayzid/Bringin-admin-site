...
const FilterJob = ({ setItems, data }) => {
  ...
  const applyFilters = () => {
    ...
    if (industryValue) {
      updatedList = updatedList.filter(
        (item) =>
          item?.expertice_area?.industryid?.industryname == industryValue
      );
    }
  ...
  return (
    <div>
      <div>
        <input type="checkbox" id="Jobpost-filter" className="modal-toggle" />
                    ...
                    <div className="flex gap-2 mt-3">
                      {subscription.map((sub, i) => (
                        <button
                          key={i}
                          onClick={() =>
                            handlesubscriptionClick(sub?.subscription)
                          }
                          className={
