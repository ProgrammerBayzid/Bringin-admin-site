...
const JobPost = () => {
  ...
  useEffect(() => {
    fetch("https://rsapp.unbolt.co/admin/job_list?job_hidden=false")
      .then((res) => res.json())
      .then((data) => {
        setIsLoding(true);
        setjobPost(data);
  ...
  const makeRejected = (_id) => {
    ...
    if (token === null) {
      ...
      try {
        fetch(`https://rsapp.unbolt.co/job_hidden_true/${_id}`, {
          method: "PATCH",
          headers:{
            Authorization: `Bearer ${token}`,
          ...
          .then((data) => {
            if (data) {
              toast.success("Job Reject Successful.");
              setRefresh(true); // Set refresh to true to trigger data fetch
              // Use setInterval to repetitively trigger data fetch every X seconds
              const fetchInterval = setInterval(() => {
                setRefresh(true);
