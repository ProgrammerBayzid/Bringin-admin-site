...
const Department = () => {
  ...
  useEffect(() => {
    ...
    fetch("http://rsapp.bringin.io/department")
      .then((res) => res.json())
      .then((data) => {
        setIsLoding(true);
        setDepartment(data);
  ...
  const handelDeeted = id => {
    ...
    if (proced) {
        fetch(`http://rsapp.bringin.io/department/${id}`, {
            method: "DELETE",
        })
            .then(res => res.json())
            .then(data => {
        ...
        <table className="min-w-full w-[1000px] text-left text-sm font-light">
          <thead className="border-b text-[18px] bgg-gray-100 font-medium dark:border-neutral-500 ">
            <tr>
              ...
              <th scope="col" className="px-6 py-4">Department ID</th>
          ...
          {
       department.map((de , i)=>
            <tbody className="text-[15px] bg-gray-50" key={de._id}
            ...
            <tr className="border-b dark:border-neutral-500">
              ...
              <td className="whitespace-nowrap px-6 py-4 font-medium">{de.name}</td>
              <td className="whitespace-nowrap px-6 py-4 font-medium">{de._id}</td>
              <td className="whitespace-nowrap px-6 py-4 text-center">
              <button  onClick={() => handelDeeted(de._id)} ><AiFillDelete className="text-red-500"></AiFillDelete></button>
