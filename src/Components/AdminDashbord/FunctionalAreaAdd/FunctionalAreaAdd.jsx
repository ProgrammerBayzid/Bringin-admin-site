...
const FunctionalAreaAdd = () => {
  ...
  useEffect(() => {
    fetch("http://rsapp.bringin.io/admin/functionalarea")
      .then((res) => res.json())
      .then((data) => {
        setIsLoding(true);
  ...
  const addFunctionalarea = (data, e) => {
    ...
    fetch("http://rsapp.bringin.io/functionalareaadd", {
      method: "POST",
      headers: {
  ...
  const handelDeeted = (id) => {
    ...
    if (proced) {
      fetch(`http://rsapp.bringin.io/admin/functionalarea/${id}`, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
                    ...
                    <div className="form-control w-full ">
                      ...
                      >
                        {category.map((ca) => {
                          ...
                          return (
                            <option value={ca._id} key={ca._id}>{ca.categoryname}</option>
                          );
                        })}
                      </select>
                    </div>
                ...
                <div className="overflow-hidden">
                  <table className="min-w-full text-left text-sm font-light w-[1000px]">
                    ...
                    {functionalarea.map((fu, i) => (
                      <tbody className="text-[15px] bg-gray-50" key={fu._id}
